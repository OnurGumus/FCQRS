...
what happens if we wantto send a command to another aggregate? In this case we need to use a saga. A saga manages a potentionally long-running 
business process which interacts with one or more external services or resources.  Sagas can include logic for timers and retries as well.
